<script lang="ts">
  ...
  const updateItem = async (
    ...
    await net(async () => {
      const encryptedProps = { ...props };
      if (props.key) encryptedProps.key = await encrypt(props.key);
      if (props.value) encryptedProps.value = await encrypt(props.value);
      await api.secrets.update({ target: id, props: encryptedProps });
                        ...
                        >
                          <div class="bg-white pr-0.5 clip-arrow-right">
                            ...
                            >
                              {#if item.editing.key}
                                <input
                                  class="-mb-0.5 border-b-2 border-white text-white outline-none"
                                  type="text"
                                  name="key"
                                  ...
                                  onkeydown={async (e) => {
                                    if (e.key === "Escape") {
                                      ...
                                      ctx.font = getComputedStyle(target).font;
                          ...
                          {#if item.editing.value}
                            <input
                              class="-mb-0.5 ml-2 flex-1 border-b-2 border-white py-1 text-white outline-none"
                              type="text"
                              name="value"
                          ...
                          {:else}
                            <!-- svelte-ignore a11y_no_static_element_interactions -->
                            <div
                              class="ml-2 flex-1 truncate py-1"
                              ondblclick={async (e) => {
                                e.preventDefault();
                                const parent = e.currentTarget.parentElement;
